...
while(1)
    ...
    if checkCollision_carBot(data, randState)
        continue
    end
...
%     end
    ...
    for k = 1: nodeListSize(1)
        if minDist > dist3d([nodeList(k, 1) nodeList(k, 2) nodeList(k, 3)], [randX randY randT])
           x = nodeList(k, 1);
           y = nodeList(k, 2);
           t = nodeList(k, 3);
           minDist = dist3d([nodeList(k, 1) nodeList(k, 2) nodeList(k, 3)], [randX randY randT]);
        end
    end
    
   
    nNeighbor = [x y t];
    minimum= dist3d([1 1 0], [400 400 360]);
    cRollOut = [];
    newNode = [];
 
    for k = 1:numcontrolsamples
        maxSteer = randi([-maxsteerangle*1000 maxsteerangle*1000]);
        sampleSteerAngle = maxSteer/1000;
        sampleLinearVel = randi([-maxlinearvel maxlinearvel]);
        
        
       [xnew, isValid, rollout] = simulate_carBot(data, nNeighbor, randState, deltastep, sampleLinearVel, sampleSteerAngle);
        if not(isValid)
           continue
        end
        rolloutSize = size(rollout);
        if rolloutSize(1) <= 1
           continue 
        end

        if minimum > dist3d(randState, xnew)
            newNode = xnew;
            cRollOut = rollout;
            minimum = dist3d([randState(1) randState(2) randState(3)], [xnew(1) xnew(2) xnew(3)]);
        end
    ...
    %Make AdjList
    adjListTemp = [];
    cRollSize = size(cRollOut);

    adjList = [adjList; nNeighbor newNode];
    for k = 1: cRollSize(1) -1     
        t3 = plot([cRollOut(k, 1) cRollOut(k+1, 1)], [cRollOut(k, 2) cRollOut(k+1, 2)]);  set(t3, 'Color', 'r'); set(t3, 'Linewidth', 2); 
    end
...
adjLSize = size(adjList);
fpath = [adjList(adjLSize(1), 4) adjList(adjLSize(1), 5) adjList(adjLSize(1), 6) adjList(adjLSize(1), 1) adjList(adjLSize(1), 2) adjList(adjLSize(1), 3)];
fPointer = 1;
adjRowSize = adjLSize(1);
for index = adjRowSize-1: -1: 1
   if fpath(fPointer, 4) == adjList(index, 4) && fpath(fPointer, 5) == adjList(index, 5) && fpath(fPointer, 6) == adjList(index, 6)
        fpath = [fpath; adjList(index, 4) adjList(index, 5) adjList(index, 6) adjList(index, 1) adjList(index, 2) adjList(index, 3)];
        fPointer = fPointer + 1;
   end
end
