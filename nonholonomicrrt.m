...
while(1)
    ...
    if goalProb >= 1 && goalProb <= 10
       for k = 1:nodeListSize
          if distance([nodeList(k, 1) nodeList(k, 2)], [goal(1) goal(2)]) < 10
    ...
    for k = 1: nodeListSize(1)
        if minDist > distance([nodeList(k, 1) nodeList(k, 2)], [randX randY]) && minAngleDist > abs(angdiff(nodeList(k, 3), randT))
           x = nodeList(k, 1);
           y = nodeList(k, 2);
           t = nodeList(k, 3);
           minDist = distance([nodeList(1) nodeList(2)], [randX randY]);
           minAngleDist = abs(angdiff(nodeList(k, 3), randT));
        end
    end
    
   
    nNeighbor = [x y t];
    minimum= distance([1 1], [400 400]);
    minAngleDist = randT;
    cRollOut = [];
    newNode = [];
    
    for k = 1:numcontrolsamples
        maxSteer = (maxsteerangle * 10);
        sampleSteerAngle = (randi([-maxSteer maxSteer]))/10.0;
       ...
       [xnew, isValid, rollout] = simulate_carBot(data, nNeighbor, randState, deltastep, sampleLinearVel, sampleSteerAngle);
        ...
        if minimum > distance([randState(1) randState(2)], [xnew(1) xnew(2)]) && minAngleDist+10 >= abs(angdiff(randState(3), xnew(3))) && minAngleDist-10 <= abs(angdiff(randState(3), xnew(3)))
            newNode = xnew;
            cRollOut = rollout;
            minimum = distance([randState(1) randState(2)], [xnew(1) xnew(2)]);
            minAngleDist = abs(angdiff(randState(3), xnew(3)));
        end
    ...
    for k = 1: cRollSize(1) -1
        adjListTemp = [cRollOut(k, 1) cRollOut(k, 2) cRollOut(k, 3) cRollOut(k+1, 1) cRollOut(k+1, 2) cRollOut(k+1, 3)];
        adjList = [adjList; adjListTemp];
    end
    adjSize = size(adjList);
    for k = 1: adjSize
        t3 = plot([adjList(k, 1) adjList(k, 4)], [adjList(k, 2) adjList(k,5)]);  set(t3, 'Color', 'r'); set(t3, 'Linewidth', 2); 
        %t1 = text(cRollOut(1), cRollOut(2), 'r*'); set(t1,'Color','r','Fontsize',20);
    end
